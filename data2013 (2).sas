OPTIONS FORMCHAR="|----|+|---+=|-/\<>*";
*--------------------------------------- plot history -----------------------------------------------------;
proc import datafile="g:\Excel Files\plothistory.csv"
out=hist 
dbms=csv replace;
getnames=yes;
run;
...
data hist2; set hist;
   ...
   * chk1 = yrsincerx - sirx;
   ...
   if (burn = 'h') then bcat1 = 'B';
   if (burn = 'm' | burn = 'l' | burn = 's') then bcat1 = 'A';
   if (burn = 'h') then bcat2 = 'C';
   if (burn = 'm') then bcat2 = 'B';
   if (burn = 's' | burn = 'l') then bcat2 = 'A';
proc print data=hist2; title 'hist2';
proc freq data=hist2; tables burn*yrsincerx bcat1*yrcat bcat2*yrcat / fisher expected;
run;

* pita - genmod;
proc genmod data=hist2; class bcat2 yrcat;
  model bcat2 = yrcat/ dist = poisson link=log type1 type3;
run;
proc univariate data

proc glm data=hist2;
  class bcat1 bcat2;
  model bcat1 = yrcat; *no significance;
...
*--------------------------------------- seedlings -----------------------------------------------------;
proc import datafile="G:\Excel Files\seedlings2013.csv"
...
data seedling4; set seedling3;
  ...
  logmhgt = log10(mhgt);
  drop plantnox mstems;
run;
proc glm data=seedling4;
  class burn bcat1 bcat2 yrcat;
  * model sumstems = bcat2 yrcat bcat2*yrcat;*no significance;
...
(mhght): Skewness = -0.6625, kurtosis = -0.3599. Shapiro-Wilk: W=0.9438, p = 0.0355;
...
*interesting--greatest freq of QUMA3 seedlings, but greatest freq of QUMA overstory. Oaks that 
...
*fix: 2 acgr, 1 cavi2 (should be cevi2), 1 coca, 1 cyre1, 4 disp, 8 lemu, check 1 soca3 
   (should be soca6?), 1 spco, 1 trur, 2 unk02;
...
* N = 2465 species-plot combinations, 44 plots;
* plantno = number of plants/plot  
  plantnox is redundant
  mstems = avage # stems/plant in the plot
  sumstems = # of stems of all species in the plot;
...
proc glm data=herbs4;
  ...
  *model logsumstems = bcat2 yrcat bcat2*yrcat;
  ...
  output out=glmout1 r=ehat;
run;
*for bcat model: more stems with greater severity. df=7. p = 0.0011. r-square = 0.4676
for burn model: same trend. df=10. p<0.0001. r-square=0.6322;

proc univariate data=glmout1 plot normal; var ehat;
run;
*log transformed is normally distributed!
Skewness = -0.3081, kurtosis = 0.0098. Shapiro-Wilk: W=0.9798, p = 0.6263;
...
proc glm data=shrubs4;
  ...
  model logsumstems = bcat2 yrcat bcat2*yrcat;
  *model logsumstems = burn yrcat burn*yrcat;
  *model plantno = bcat2 yrcat bcat2*yrcat;
  *model logplantno = bcat2 yrcat bcat2*yrcat; 
  lsmeans burn*yrcat / stderr;
  output out=glmout1 r=ehat;
run;
*burn model: fewer stems with heavier burns. df = 10, p = 0.0072, r-square = 0.5341
for all--fewer stems and plants with heavier burns;
